...
contains
  ...
  subroutine create_netcdf(ncfile,ncid,xdimid,ydimid,tdimid,&
    ...
    status = nf90_def_dim(ncid = ncid, name = "x", len = xdim+2*gc, dimid = xdimid)
    if (status /= NF90_NOERR) call handle_err(status,"Creating x failed.")
    status = nf90_def_dim(ncid = ncid, name = "y", len = ydim+2*gc, dimid = ydimid)
  ...
  subroutine write_constant_values(ncid,xid,yid,phioroid,phibarid,latid)
    ...
    integer :: status
    real,dimension(ydim+2*gc,xdim+2*gc) :: phioros
    ...
    ! write orography
    phioros = swap_dimensions(phioro)      
    status = nf90_put_var(ncid = ncid, varid = phioroid, values = phioros, start = (/ 1, 1 /), count = (/ ydim+2*gc, xdim+2*gc /))
    if (status /= NF90_NOERR) call handle_err(status,"Could not write orographic height data.")

    ! write grid
    status = nf90_put_var(ncid = ncid, varid = xid, values = x, start = (/ 1, 1 /), count = (/ xdim+2*gc /))
    if (status /= NF90_NOERR) call handle_err(status,"Could not write grid data (x dimension).")      
    status = nf90_put_var(ncid = ncid, varid = yid, values = y, start = (/ 1, 1 /), count = (/ ydim+2*gc /))
  ...
  subroutine write_timestep(counter,ncid,tid,uid,vid,phiid,exactid,eps1id,eps2id,eps3id)
    ...
    integer :: status
    real,dimension(ydim+2*gc,xdim+2*gc) :: vartrans
    real,dimension(ydim+2*gc,xdim+2*gc,1) :: vart
    ...
    vartrans = swap_dimensions(u)
    vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
    status = nf90_put_var(ncid = ncid, varid = uid, values = vart, start = (/ 1, 1, counter /),&
         &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
    ...
    vartrans = swap_dimensions(v)
    vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
    status = nf90_put_var(ncid = ncid, varid = vid, values = vart, start = (/ 1, 1, counter /),&
         &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
    ...
    vartrans = swap_dimensions(phi)
    vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
    status = nf90_put_var(ncid = ncid, varid = phiid, values = vart, start = (/ 1, 1, counter /),&
         &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
    ...
    if (test.ne.0) then
       vartrans = swap_dimensions(exact)
       vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
       status = nf90_put_var(ncid = ncid, varid = exactid, values = vart, start = (/ 1, 1, counter /),&
            &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
    ...
    vartrans = swap_dimensions(epsilon(:,:,1))
    vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
    status = nf90_put_var(ncid = ncid, varid = eps1id, values = vart, start = (/ 1, 1, counter /),&
         &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
    if (status /= NF90_NOERR) call handle_err(status,"Could not write local off-centring parameter epsilon1 for time step.")
 
    ! write off-centring parameters for this time step
    vartrans = swap_dimensions(epsilon(:,:,2))
    vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
    status = nf90_put_var(ncid = ncid, varid = eps2id, values = vart, start = (/ 1, 1, counter /),&
         &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
    if (status /= NF90_NOERR) call handle_err(status,"Could not write local off-centring parameter epsilon2 for time step.")
 
    ! write off-centring parameters for this time step
    vartrans = swap_dimensions(epsilon(:,:,3))
    vart = reshape(source = vartrans, shape = (/ ydim+2*gc, xdim+2*gc, 1 /))
    status = nf90_put_var(ncid = ncid, varid = eps3id, values = vart, start = (/ 1, 1, counter /),&
         &count = (/ ydim+2*gc, xdim+2*gc, 1 /))
